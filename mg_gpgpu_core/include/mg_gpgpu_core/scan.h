...
/**This is a gpu kernel that implements a naive hillis and steel scan kernel,
 * which is not work efficient. This kernel only reduce for N hops away for each
 * thread, meaning this kernel needs to be called several times in order to
 ...
 */
template <typename T>
__global__ void parallel_scan_hillis_steel_kernel(const T *in, T *out,
                                                  uint32_t hop,
                                                  uint32_t count) {
...
inline T *parallel_scan_hillis_steel(T *d_in, T *d_out, uint32_t count) {
  ...
  for (int hop = 1; hop < count; hop <<= 1) {
    std::swap(d_in, d_out);
    parallel_scan_hillis_steel_kernel<T>
        <<<blocks, threads>>>(d_in, d_out, hop, count);
...
parallel_scan_blelloch_kernel(T *d_in, int lg, uint32_t count, int blockId) {
...
/**
 * @brief wrapper function for the blellock_array_kernel, to kick one for each
 * block
 */
template <typename T>
__global__ void parallel_scan_blelloch_wrap_kernel(T *d_in, uint32_t lg,
                                                   uint32_t count) {
  parallel_scan_blelloch_kernel<T>(d_in, lg, count, blockIdx.x);
}

template <typename T, T SENTINEL_VALUE>
__device__ void intra_block_barrier(int tid, int gid,
                                    volatile T *d_intermediate, T curr_value,
                                    T *out) {
  T p = 0;
  if (tid == 0) {
    if (gid == 0) {
...
__global__ void parallel_stream_scan_kernel(T *d_in, volatile T *d_intermediate,
                                            T *atom, uint32_t count,
                                            uint32_t blocks) {
  ...
  __shared__ T prev;
  intra_block_barrier<T, SENTINEL_VALUE>(threadIdx.x, _gId, d_intermediate, res,
                                         &prev);
...
/**
 * @brief performs a scan inclusive, using dynamic blocks allocation and spin
 * locking
 *
 * This kernel is based on the paper:
 * http://dl.acm.org/citation.cfm?id=2442539
 * The main idea behind it, is you move the syncronization point in the
 * algorithm as early as possible, in this case, we use a reduce in the blocks
 * to feed the result in the next block. To make sure we process the blocks
 * increasing order atomic id are generated and spin lock in global memory is
 * used to stall the blocks, waiting for the previous reduce
 *
 * @tparam T data type to use in the algorithm
 * @tparam SENTINEL_VALUE T value we want to use for the spin lock, as long this
 * value is in memory the block won't proceed any further
 * @tparam WARP_SIZE the size of a warp usually 32
 * @param d_in device memory, to note this memory is not constant, and scan is
 * in place
 * @param d_intermediate intermediate device memory, this array has the same
 * size as number of blocks
...
inline void parallel_stream_scan(T *d_in, T *d_intermediate, uint32_t count) {
  ...
  uint32_t blocksSet = ((blocks / threadsSet) != 0) ? (blocks / threadsSet) + 1
                                                    : (blocks / threadsSet);
  if (blocksSet == 0) {
    blocksSet = 1;
  }
  ...
  // setting memory
  mg_gpgpu::utils::set_value_kernel<T, SENTINEL_VALUE>
      <<<blocksSet, threadsSet>>>(d_intermediate, blocks);
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess)
    printf("Error: %s\n", cudaGetErrorString(err));

  // zeroing out last value , this might be optimized a little by having a
  // bespoke kernel
  ...
  // here we use the shuffle block reduce, where each block reduces a warp in
  // shared memory and then performs  a final shuffle, intermediate result stored
  // in shared memory
  uint32_t sharedMemorySize = (threads / WARP_SIZE) * sizeof(T);

  // kicking the kernel
  parallel_stream_scan_kernel<T, SENTINEL_VALUE, EXCLUSIVE>
      <<<blocks, threads, sharedMemorySize>>>(
          d_in, d_intermediate, d_intermediate + blocks, count, blocks);
  err = cudaGetLastError();
  if (err != cudaSuccess)
    printf("Error: %s\n", cudaGetErrorString(err));
}
...
std::unique_ptr<T[]> parallel_scan_hillis_steel_alloc(T *data, uint32_t count) {
  ...
  gpuErrchkDebug(
      cudaMemcpy(in, data, count * sizeof(T), cudaMemcpyHostToDevice));
  ...
  gpuErrchkDebug(cudaMemcpy(ptr.get(), final_ptr, count * sizeof(T),
...
std::unique_ptr<T[]> parallel_stream_scan_alloc(T *data, uint32_t count) {
  ...
  gpuErrchkDebug(
      cudaMalloc((void **)&d_intermediate, (blocks + 1) * sizeof(T)));
  ...
  auto ptr = std::unique_ptr<T[]>(new T[count]);
  gpuErrchkDebug(
      cudaMemcpy(ptr.get(), d_in, count * sizeof(T), cudaMemcpyDeviceToHost));

  return ptr;
}

template <typename T> void parallel_scan_blelloch(T *d_in, uint32_t count) {
  // one block with given size of element, this kernel is supposed to work on
  // one block only
  uint32_t threads = count;
  uint32_t blocks = 1;

  // computing the log needed
  // TODO not really an expensive operation but would be nice to try compute it
  // myself,  leveraging the fact (maybe) i know is a power of two
