...
#define gpuErrchk(ans)                                                         \
  { mg_gpgpu::utils::gpuAssert((ans), __FILE__, __LINE__); }
...
#define gpuErrchkDebug(ans)                                                    \
  { mg_gpgpu::utils::gpuAssertDebug((ans), __FILE__, __LINE__); }
...
/// param size: size of the buffer
// TODO(giordi) use vectorized load or stores
...
// TODO(giordi) Make it grid size loop
// TODO(giordi) use vectorized load or stores
template <typename T>
...
//TODO(giordi) let the block size be set by the user, either as template param
//or regular arg
template <typename T> void copy(T *d_in, T *d_out, unsigned int count) {
...
//////////////////////////////////////////////////////////////////////////////////////
// alloc version for the zero out kernel, for documentation check the non alloc
// version
// of the kernel
/// param data: pointer to the device data to zero out
/// param size: size of the buffer
...
/// param threads: how many threads per block
template <typename T, T VALUE>
std::unique_ptr<T[]> set_value_alloc(T *data, unsigned int size,
                                     unsigned int threads = 512) {
